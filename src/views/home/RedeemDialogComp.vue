    ...
    <div class="base-dialog_wrapper" @click.stop>
      ...
      <div class="base-dialog_body">
        <div class="base-dialog-body_input">
          <div><span>质押</span><span>已质押:{{ balance }}</span></div>
          <div>
            <input v-model="number"/>
            <div><button @click="number=farmList[0].pledgeAmount">最大</button><span>LP</span></div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    disabled () {
      return isNaN(this.number) || Number(this.number) <= 0 || Number(this.number) > this.farmList[0].pledgeAmount
    },
    farmList () {
  ...
  methods: {
    ...
    submit () {
      ...
      this.$emit('update:visible', false)
      this.farmContract.redeem({ number: this.number }).then(hash => {
